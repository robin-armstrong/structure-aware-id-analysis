...
if(!plot_only)
    function run_crossecs(destination, readme, rng, n, decay_Start, decay_end, sigma_start, sigma_end, krange, num_cohers, k_cross, c_cross, numtrials)
        ...
        ranktest_coher = zeros(length(krange))

        for i = 1:length(krange)
            k                 = krange[i]
            ov                = ceil(Int64, .1*k)
            lscores           = [norm(Vt[1:k, j])^2 for j = 1:n]
            lscores           = min.(lscores, 1.)
            lscores           = max.(lscores, 0.)
            ranktest_coher[i] = sqrt(maximum(lscores))
            ...
            @save destination*"_data.jld2" krange cohers k_cross c_cross ranktest_coher numtrials S data_vsrank means_vsrank stds_vsrank quants_vsrank data_vscoher means_vscoher stds_vscoher quants_vscoher
        end
...
@load destination*"_data.jld2" krange cohers k_cross c_cross ranktest_coher numtrials S data_vsrank means_vsrank stds_vsrank quants_vsrank data_vscoher means_vscoher stds_vscoher quants_vscoher
...
ioff()
fig, (vscoher, ranktest_coher_profile, vsrank_opt, stablerank) = subplots(2, 2, figsize = (8, 8))
...
ranktest_coher_profile.set_xlabel(L"Approximation Rank ($k$)")
ranktest_coher_profile.set_ylabel(L"Coherence ($c_k$)")
...
ranktest_coher_profile.plot(krange, ranktest_coher, color = "black")
